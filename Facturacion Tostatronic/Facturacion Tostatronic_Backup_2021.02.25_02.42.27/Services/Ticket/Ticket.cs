...
namespace Facturacion_Tostatronic.Services.Ticket
{
    class Ticket
    {
        public Ticket()
        {
            myDocument.AddAuthor("Tostatronic - Software desing");
            myDocument.AddCreator("Jorge Alvarado");
        ...
        Document myDocument = new
            Document(new Rectangle(200,843)); //Aqui se ponen todos los objetos
        string line = "";
        #region Properties
            ...
            try
            {
                if ((headerImage != null) && (headerImage != ""))
                {
                    if (File.Exists(headerImage))
                    {
                        ...
                        logo.SetAbsolutePosition(60,
                            (myDocument.PageSize.Height - imageHeight)+5);
        ...
        private void DrawItems()
        {
            ...
            {
                cb.SetTextMatrix(leftMargin, YPosition());
                cb.ShowText("CANT DESCRIPCION        PRECIO   IMPORTE");
            }
            count++;
            DrawEspacio();
            int espacios = 10;
            foreach (string item in items)
            {
                ...
                cb.SetTextMatrix(leftMargin, YPosition());
                cb.ShowText(line);
                //espacios -= line.Length;
                
                espacios = 24;
                //espacios -= name.Length;
                line = ordIt.GetItemPrice(item);
                line = AddSpaces(espacios+line.Length, line);
                cb.SetTextMatrix(leftMargin, YPosition());
                cb.ShowText(line);
                espacios = 33;
                //espacios -= line.Length;
                ...
                if (name.Length > maxCharDescription)
                {
                    ...
                    int start = 0;
                    int numC = maxCharDescription;
                    string aux = string.Empty;
                    while (start < name.Length)
                    {
                        aux = name.Substring(start, numC);
                        start += numC;
                        numC = name.Length - start;
                        if (numC > maxCharDescription)
                        {
                            if ((numC - maxCharDescription) > 10)
                                numC = maxCharDescription + 11;
                            else if ((numC - maxCharDescription) > 5)
                                numC = maxCharDescription + 6;
                            else
                                numC = maxCharDescription + 1;
                        }
                        cb.SetTextMatrix(leftMargin, YPosition());
                        line = AddSpaces(espacios + aux.Length, aux);
                        cb.ShowText(line);
                        count++;
                    }
                }
                ...
                {
                    name = AddSpaces(espacios + name.Length, name);
                    cb.SetTextMatrix(leftMargin, YPosition());
                    cb.ShowText(name);
                }
        ...
        private void DrawTotales()
        {
            ...
            foreach (string total in totales)
            {
                ...
                line = AddSpaces(40, line);
                cb.SetTextMatrix(leftMargin, YPosition());
                cb.ShowText(line);
                leftMargin = 0;
                line = ordTot.GetTotalName(total);
                line = AddSpaces(29, line);
                cb.SetTextMatrix(leftMargin, YPosition());
                cb.ShowText(line);
                count++;
            }
